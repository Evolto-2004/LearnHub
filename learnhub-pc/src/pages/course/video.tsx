...
const CoursePalyPage = () => {
  ...
  const initDPlayer = (playUrl: string, isTrySee: number, params: any) => {
    ...
    window.player.on("timeupdate", () => {
      ...
      if (
        ...
        message.warning("Fast forwarding is disabled during first-time learning");
  ...
  const goNextVideo = () => {
    ...
    if (index === totalHours.length - 1) {
      setIsLastpage(true);
      message.error("This is the last lesson!");
    ...
    <div className={styles["video-mask"]}>
      <div className={styles["top-cont"]}>
        <div className={styles["box"]}>
          ...
          >
            <ArrowLeftOutlined />
            <span className="ml-14">Back</span>
          </div>
        </div>
      </div>
          ...
          {checkPlayerStatus && (
            <div className={styles["alert-message"]}>
              <div className={styles["des-video"]}>
                A new video is already open. This video has been paused.
              </div>
            </div>
          )}
          {playendedStatus && (
