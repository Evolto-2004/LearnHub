  ...
  const uploadProps = {
    ...
    beforeUpload: async (file: File, fileList: any) => {
      if (file.size > 2 * 1024 * 1024 * 1024) {
        message.error(`${file.name} exceeds the 2 GB size limit`);
        return Upload.LIST_IGNORE;
      }
      if (fileList.length > 10) {
        message.config({ maxCount: 1 });
        message.error("You can upload up to 10 videos at a time");
        return Upload.LIST_IGNORE;
      } else {
        message.config({ maxCount: 10 });
      }
      if (file.type === "video/mp4") {
        // Video封面解析 || Video Duration解析
        ...
        message.error(`${file.name} is not an MP4 video file`);
      }
      return Upload.LIST_IGNORE;
    },
  };
      ...
      {uploadStatus ? (
        <>
          ...
          >
            <img src={upIcon} />
            <span>
              VideoUpload succeeded ({successNum}/{fileList.length})
            </span>
          </div>
          <Modal
            width={800}
            ...
            <Row gutter={[0, 10]}>
              <Col span={24}>
                <Dragger {...uploadProps}>
                  ...
                  <p className="ant-upload-text">Drag videos here to upload</p>
                  <p className="ant-upload-hint">
                    Supports multiple uploads / MP4 files up to 2 GB
                  </p>
                </Dragger>
              </Col>
              <Col span={24}>
                      ...
                      render: (_, record) => (
                        <>
                          ...
                          {record.upload.status === 7 ? (
                            <Tag color="success">Upload succeeded</Tag>
                          ) : null}
                        </>
                      ),
                    },
