...
const CoursePage = () => {
  ...
  const getDetail = () => {
    vod.detail(Number(courseId)).then((res: any) => {
      ...
      document.title = courseItem.title || "Course Details";
  ...
  useEffect(() => {
    if (course) {
      setCourseTypeText(course.is_required === 1 ? "Required" : "Optional");
    }
  }, [course]);
  ...
  const downLoadFile = (cid: number, id: number, rid: number) => {
    vod.downloadAttachment(cid, id).then((res: any) => {
      ...
      if (isWechat()) {
        if (isIOS()) {
          Toast.show("Please use the browser menu in the top right to download.");
        }
  ...
  return (
    <div className="main-body">
      <a
        ...
        download={true}
      ></a>
      <div className="main-header" style={{ backgroundColor: "#2563EB" }}>
        <Image
          className="back-icon"
          src={backIcon}
          onClick={() => {
